<template>
  <div :class="`${themes[themeStatus]}`">
    ...
    <el-table :data="tableData" stripe @selection-change="handleSelectionChange" :header-cell-style="{'padding-left':'4px','padding-right':'4px'}">
      ...
      <el-table-column label="商品详情"><template v-slot="scope"><el-button type="primary" @click="views(scope.row.des)">查看</el-button></template></el-table-column>
      <el-table-column prop="stockNum" label="剩余库存"></el-table-column>
      <el-table-column prop="stockOutNum" label="已卖出"></el-table-column>
      <el-table-column prop="checkStatus" label="上下架">
    ...
    <el-dialog title="商品信息" :visible.sync="dialogFormVisible" width="40%" :close-on-click-modal="false">
      <el-form label-width="140px" size="small" style="width: 85%;" :model="form" :rules="rules" ref="ruleForm">
        ...
        <el-form-item prop="goodsInfoInfoContent" label="商品详情">
          <mavon-editor ref="md" v-model="form.des" :ishljs="true" @imgAdd="imgAdd"/>
        </el-form-item>
        <el-form-item prop="checkStatus" label="上下架">
          <el-select v-model="form.checkStatus" placeholder="请选择上下架">
            <el-option
...
export default {
  ...
  methods: {
    ...
    exp() {
      window.open(this.serverHost+"/goods/export")
    },
