<template>
  <div :class="`${themes[themeStatus]}`">
    ...
    <el-table :data="tableData" stripe @selection-change="handleSelectionChange" :header-cell-style="{'padding-left':'4px','padding-right':'4px'}">
      ...
      <el-table-column label="商品详情"><template v-slot="scope"><el-button type="primary" @click="goodsInfoViews(scope.row.id)">查看</el-button></template></el-table-column>
      <el-table-column label="地址详情"><template v-slot="scope"><el-button type="primary" @click="addressViews(scope.row.addressId)">查看</el-button></template></el-table-column>
      ...
      <el-table-column label="操作"  width="180" align="center">
        <template v-slot:="scope">
          <div class="table-row-options-warp">
            <span @click="handleEdit(scope.row)">发货 </span>
          </div>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    load() {
      this.request.get("/orderInfo/findPage", {
        params: {
