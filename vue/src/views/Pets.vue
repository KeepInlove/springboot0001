...
<!--      <el-upload :action="serverHost+'/pets/import'" :show-file-list="false" accept="xlsx" :on-success="handleExcelImportSuccess" style="display: inline-block">-->
...
<!--      <el-button type="primary" @click="exp" class="ml-5">导出 <i class="el-icon-top"></i></el-button>-->
    ...
    <el-table :data="tableData" stripe @selection-change="handleSelectionChange" :header-cell-style="{'padding-left':'4px','padding-right':'4px'}">
      ...
      <el-table-column label="宠物封面照"><template v-slot="scope"><el-image style="width: 100px; height: 100px" :src="scope.row.img" :preview-src-list="[scope.row.img]"></el-image></template></el-table-column>
      <el-table-column label="宠物详情"><template v-slot="scope"><el-button type="primary" @click="views(scope.row.petsContent)">查看</el-button></template></el-table-column>
      <el-table-column prop="cateName" label="所属分类名称"></el-table-column>
    ...
    <el-dialog title="信息" :visible.sync="dialogFormVisible" width="40%" :close-on-click-modal="false">
      <el-form label-width="140px" size="small" style="width: 85%;" :model="form" :rules="rules" ref="ruleForm">
        ...
        <el-form-item prop="img" label="宠物封面照">
          <el-upload :action="serverHost+'/file/upload'" ref="img" :on-success="handleImgUploadSuccess">
            <el-button size="small" type="primary">点击上传</el-button>
          </el-upload>
        </el-form-item>
        <el-form-item prop="petsContent" label="宠物详情">
          <mavon-editor ref="md" v-model="form.petsContent" :ishljs="true" @imgAdd="imgAdd"/>
        </el-form-item>
        <el-form-item prop="cateId" label="所属分类">
...
export default {
  ...
  methods: {
    ...
    save() {
      const category = this.category.find(item => item.id === this.form.cateId);
      this.form.cateName = category ? category.name : '';
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          this.request.post("/pets", this.form).then(res => {
            if (res.code === '200') {
    ...
    del(id) {
      this.request.delete("/pets/" + id).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.request.post("/pets/del/batch", ids).then(res => {
        if (res.code === '200') {
    ...
    exp() {
      window.open(this.serverHost+"/pets/export")
    },
